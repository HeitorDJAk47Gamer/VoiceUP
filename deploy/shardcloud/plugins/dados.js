...
function parseExpression(expression) {
  const terms = []; const token = /\s*([+-]?)\s*(?:(\d{0,3})d(\d{1,5})|(\d{1,5}))/iy; let cursor = 0;
  while (cursor < expression.length) {
    token.lastIndex = cursor; const match = token.exec(expression); if (!match) return [];
...
function describeTerm(term, index) {
  ...
  return `${prefix}${term.dice === 1 ? '' : term.dice}d${term.faces} [${term.rolls.join(', ')}]`;
}

module.exports = {
  id: 'dados', name: 'Dados RPG', version: 'beta.3', icon: ICON,
  ...
  onTextMessage({ text, room, textChannel, user, api, plugin }) {
    const match = String(text).match(expressionPattern); if (!match) return;
    const expression = match[1]; const terms = parseExpression(expression);
    ...
    for (const term of terms) {
      ...
      term.value = term.rolls.reduce((sum, value) => sum + value, 0); total += term.sign * term.value;
    }
    api.systemMessage(room, textChannel, `🎲 ${user.name} rolou ${expression}: ${terms.map(describeTerm).join('')} = ${total}`, { name: 'Dados RPG', color: '#e8b65a', avatar: ICON, pluginId: plugin.id });
  }
};
