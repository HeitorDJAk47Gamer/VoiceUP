...
io.on('connection', (socket) => {
  ...
  socket.on('text-message', ({ text, textChannel }) => { if (!socket.data.serverRoom) return; const safeText = String(text || '').trim().slice(0, 500); if (!safeText) return; const safeTextChannel = safeChannel(textChannel, 'geral'); io.to(socket.data.serverRoom).emit('text-message', { from: socket.id, text: safeText, textChannel: safeTextChannel, name: socket.data.name || 'Visitante', color: socket.data.color || colors[0] }); plugins.onTextMessage({ text: safeText, room: socket.data.room, textChannel: safeTextChannel, user: { id: socket.id, name: socket.data.name || 'Visitante', color: socket.data.color || colors[0] }, serverIsCloud: true }); });
