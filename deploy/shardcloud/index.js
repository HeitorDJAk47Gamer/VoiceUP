...
const peersIn = (key) => [...(io.sockets.adapter.rooms.get(key) || [])].map((id) => { const peer = io.sockets.sockets.get(id)?.data || {}; return { id, name: peer.name || 'Visitante', color: peer.color || colors[0], avatar: peer.avatar || '' }; });
io.on('connection', (socket) => {
  socket.on('join-room', ({ roomId, voiceChannel, name, color, avatar }) => {
    ...
    socket.emit('color-assigned', { color: safeColor }); const peers = peersIn(voiceRoom).filter((peer) => peer.id !== socket.id); socket.emit('room-joined', { roomId: room, voiceChannel: channel, peers }); socket.to(voiceRoom).emit('peer-joined', { id: socket.id, name: safeName, color: safeColor, avatar: safeAvatar });
  });
  ...
  socket.on('disconnecting', () => { if (socket.data.voiceRoom) socket.to(socket.data.voiceRoom).emit('peer-left', { id: socket.id, name: socket.data.name }); });
});
